...
subroutine rotation_analysis
  ...
  if(GR) then
     ...
     !!!$omp target teams distribute parallel do map(to: X, W, rho, volume, vphi, x1, eps, press, mgrav, mass) map(tofrom: ToverW) &
     !!!$omp reduction(+:angular_momentum, rotational_energy, internal_energy) map(to: ghosts1, n1)
     do i = ghosts1+1, n1-ghosts1
        ...
        ToverW(i) = rotational_energy / abs(mgrav(i) - mass(i) - internal_energy - rotational_energy)
     enddo

...
subroutine flux_differences_hlle
  ...
  if (GR) then
     !$omp target teams distribute parallel do map(tofrom: smin, smax) map(to: eigenvaluesl, eigenvaluesr, n1)
     do i=1,n1-1
  ...
  !$omp target teams distribute parallel do map(to: smin, smax, n1, n_cons) map(tofrom: dspeed, flux_diff)
  do i=1,n1

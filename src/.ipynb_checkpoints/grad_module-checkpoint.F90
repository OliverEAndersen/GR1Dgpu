...
function Gradient(var,r) result(grad)
  ...
  !$omp target teams distribute parallel do map(to: var, r, ghosts1, n1) map(tofrom: grad)
  do i=ghosts1+2,n1-ghosts1-1
...
function Gradient_int(var,radius) result(grad)
  ...
  !$omp target teams distribute parallel do map(to: var, radius, ghosts1, n1) map(tofrom: grad)

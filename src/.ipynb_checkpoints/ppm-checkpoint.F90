...
contains
  ...
  subroutine ppm_coefficients
    ...
    !$omp target teams distribute parallel do &
    !$omp map(to: delta_r, ghosts1, n1) &     
    !$omp map(tofrom: tmp_1, tmp_2, tmp_3)
    do i=ghosts1-2,n1-1 
    ...
    !$omp target teams distribute parallel do &
    !$omp map(to: delta_r, tmp_1, tmp_2, tmp_3, ghosts1, n1) &
    !$omp map(tofrom: c_1_r, c_2_r) 
    do i=ghosts1-1,n1-2
       tmp = delta_r(i) / &
    ...
    ! a_i+1 - a_i (1.6)
    !$omp target teams distribute parallel do &
    !$omp map(to: tmp_1, delta_r, tmp_2, tmp_3, ghosts1, n1) &  
    !$omp map(tofrom: c_3_r, c_4_r, c_5_r) 
    do i=ghosts1-1,n1-3
  ...
  subroutine ppm_interpolate(q,qp,qm)
    ...
    ! ppm_tmp(i) = a_i+1 - a_i
    !$omp target teams distribute parallel do &
    !$omp map(to: q, ghosts1, n1) &           
    !$omp map(tofrom: ppm_tmp)                
    do i = ghosts1-2,n1-1
       ppm_tmp(i) = q(i+1) - q(i)
    enddo
    !$omp end target teams distribute parallel do
    ...
    ! a_i+1/2 (1.6)
    !$omp target teams distribute parallel do &
    !$omp map(to: q, c_5_r, c_3_r, c_4_r, delta_q, ghosts1, n1) &  
    !$omp map(tofrom: qp, qm) 
    do i=ghosts1-1,n1-3
       qp(i) = q(i) + &
            c_5_r(i) * ppm_tmp(i) + &
            c_3_r(i) * delta_q(i+1) + &
  ...
  subroutine ppm_steepen
    ...
    !$omp target teams distribute parallel do &
    !$omp map(to: c_1_r, rho, ghosts1, n1) &       
    !$omp map(tofrom: delta_q) 
    do i=ghosts1-2,n1-2

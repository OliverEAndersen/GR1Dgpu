...
subroutine prim2con_1
  ...
  if (GR) then 
     ...
     enddo
     if(do_rotation) then
        !$omp target teams distribute parallel do map(tofrom: q) map(to: rho, vphi1, x1) map(to: n1)
        do i=1,n1
     ...
     if(activate_turbulence) then
       ...
       !$omp target teams distribute parallel do map(tofrom: q) map(to: rho, v_turb) map(to: n1)
       do i=1,n1
